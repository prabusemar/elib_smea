...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Validasi input
    if (empty($nama) || empty($email) || empty($password) || empty($jabatan)) {
        ...
        if (mysqli_num_rows($check) > 0) {
            ...
            if (empty($error)) {
                ...
                try {
                    ...
                    $sql_staff = "INSERT INTO staff (Nama, Email, Password, FotoProfil, TanggalBergabung, Jabatan, Status) 
                                  VALUES (?, ?, ?, ?, CURDATE(), ?, 'Active')";
                    $stmt = mysqli_prepare($conn, $sql_staff);
                    mysqli_stmt_bind_param($stmt, "sssss", $nama, $email, $hashed_password, $foto_profil, $jabatan);
                    mysqli_stmt_execute($stmt);

                    // Insert ke tabel users
                    $sql_users = "INSERT INTO users (username, password, role) VALUES (?, ?, 'staff')";
                    ...
                    $error = 'Gagal menambahkan staff: ' . $e->getMessage();
...
<div class="form-container">
    <div class="form-header">
        <h1><i class="fas fa-user-plus"></i> Tambah Staff Baru</h1>
        <a href="staff_admin.php" class="back-button">
            ...
            <div class="form-column">
                <div class="form-group">
                    ...
                    <div class="image-preview">
                        <img id="previewFoto" src="../../assets/profiles/default.jpg" alt="Preview Foto Profil">
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <select id="jabatan" name="jabatan" required>
                    ...
                    <option value="Librarian" <?= ($_POST['jabatan'] ?? '') === 'Librarian' ? 'selected' : '' ?>>Librarian</option>
                    <option value="Manager" <?= ($_POST['jabatan'] ?? '') === 'Manager' ? 'selected' : '' ?>>Manager</option>
                    <option value="IT Support" <?= ($_POST['jabatan'] ?? '') === 'IT Support' ? 'selected' : '' ?>>IT Support</option>
                </select>
            </div>
        </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        fileInput.addEventListener('change', function() {
            ...
            if (file) {
                ...
                previewImg.src = '../../assets/profiles/default.jpg';
            }
        });
    });
</script>